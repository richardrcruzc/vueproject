...
class AdminReadingView(AdminView):
    NUM_PAGES = 30

    def get(self, request, **kwargs):
        self.set_page('reading', 'Senarai Bacaan')

        if 'page_action' in kwargs:
            if kwargs['page_action'] == 'add':
                self.set_page('reading', 'Tambah Bacaan')
            ...
            if request.user.groups.filter(name__in=['admin']).exists():
                if new_filter == 0:
                    ...
                    readings = ReadingMaterial.objects.filter(tags__name__regex=r''+filter_regex[1:],
            ...
            for reading in shown_readings.object_list:
                tag = reading.tags.first()

                if reading.status == 10:
                    reading_status = 'Terbit'
                elif reading.status == 5:
                    reading_status = 'Tinjauan'
                else:
                    reading_status = 'Draf'
                questions = ReadingQuestion.objects.filter(reading_material=reading).all()
                r_video_upload = ReadingFile.objects.filter(reading_material=reading).all()
                tq = ReadingQuestion.objects.filter(reading_material=reading).all()
                q_video_upload = ReadingQuestion.objects.filter(reading_material=reading, tutorial_file='').all()
                queston_video_t = len(tq)
                queston_video = len(q_video_upload)
                reading_video = len(r_video_upload)
                uploads = queston_video_t + reading_video - queston_video
                reading_list.append({
                    'id': reading.id,
            ...
            self.context.update({'reading_data': reading_list})

            shown_pages = 2

            # check first page
            paginator_range_first = shown_readings.number - shown_pages
            if paginator_range_first < 3:
                ...
                paginator_range_last = shown_readings.number + shown_pages
            ...
            return self.render(request, 'arc/admin/reading-list.html')
